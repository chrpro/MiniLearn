...
int main(void)
{
    ...
    for (int index = 0; index < filterSize * numberOFilters; index++)
    {
        ...
        conv3_w_2[index] = conv3_w[index];
        // printf("%d, ",conv3_w[index]);
    }
    for (int index = 0; index < numberOFilters; index++)
    {
        conv3_b_2[index] =  conv3_b[index];
    }

    int PoolOut = numberOFilters*CONV3_OUT_DIM*CONV3_OUT_DIM;
    ...
    for(int img_row=0; img_row < NODE_0_TOTAL_TRAIN_IMAGES; img_row++)
    {
        ...
        for (int i = 0; i < PoolOut; i++){
            q_X0.data[ img_row * q_X0.cols + i] = (float) conv3_out[i] /  powf(2,CONV3_OUT_Q) ;        
        }
    ...
    for(int img_i=0; img_i < TOTAL_TEST_IMAGES; img_i++)
    {
        ...
        for (int i = 0; i < PoolOut; i++){
            X_test.data[ img_i*X_test.cols + i] = (float) conv3_out[i] /  powf(2,CONV3_OUT_Q) ;
        }
