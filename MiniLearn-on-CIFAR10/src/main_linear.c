...
int main(void)
{
    const char chalmersBanner[] = {
		"============================================================\n"
		"On-line Learning on low-power IoT devices                   \n"
    ...
    uint32_t index = 0;
    // int PoolOut = CONV3_OUT_DIM* CONV3_OUT_DIM*CONV3_OUT_CH;
     int PoolOut = POOL3_OUT_DIM* POOL3_OUT_DIM*CONV3_OUT_CH;
    ...
    for(int img_row=0; img_row < NODE_0_TOTAL_TRAIN_IMAGES; img_row++)
    {
        ...
        for (int i = 0; i < PoolOut; i++){
            q_X0.data[ img_row * q_X0.cols + i] = (float) pool3_out[i] /  powf(2,CONV3_OUT_Q) ;        
        }
    ...
    for(int img_i=0; img_i < TOTAL_TEST_IMAGES; img_i++)
    {
        ...
        for (int i = 0; i < PoolOut; i++){
            X_test.data[ img_i*X_test.cols + i] = (float) pool3_out[i] /  powf(2,CONV3_OUT_Q) ;
        }
    ...
    int convout = CONV3_OUT_CH*POOL3_OUT_DIM*POOL3_OUT_DIM;
  
    active_learning.layers[0] = make_connected_layer(convout, INTERFACE_OUT); 
    
    // for (int index = 0; index < INTERFACE_WT_SHAPE; index++)
    // {
    //     active_learning.layers[1].w.data[index] = (float) interface_w[index] /  powf(2, INTERFACE_WEIGHT_Q);
    // }
    
    // for (int index = 0; index < INTERFACE_BIAS_SHAPE; index++)
    // {
    //     active_learning.layers[1].b.data[index] = (float)  interface_b[index] /  powf(2, INTERFACE_BIAS_Q);
    // }
