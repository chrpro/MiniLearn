...
###################################################################################################
    ...
    def register_logging(self):
        for module in self.model.named_children():
            if isinstance(module[1], (nn.Conv2d, nn.Linear)):
                ...
                print("quantize " + str(self.param_prefix_name) +' q = ' + str( self.q_params[self.param_prefix_name]["OUT_Q"] ) )
                print('max = : ' +str(torch.max(module[1].output)) + ' min: '+str(torch.min(module[1].output)) )
            if isinstance(module, nn.MaxPool2d):
                self.param_prefix_name = module[0]
                self.logging[self.param_prefix_name + "_OUT"] = \
                    self.quantize_tensor(
                        module[1].output,
                        self.compute_fractional_bits_tensor(module[1].output)
                        ).numpy()
    ...
    def draw_model_comparison(self, draw=False):
        for module in self.model.named_children():
            self.param_prefix_name = module[0]
            if isinstance(module[1], (nn.Conv2d, nn.Linear,  nn.MaxPool2d)):
                if draw:
                    draw_activation(
                        os.path.join(
                            self.io_folder,
